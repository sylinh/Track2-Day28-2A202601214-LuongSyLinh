...
def dedupe_latest(events: Iterable[IngestionEvent]) -> list[IngestionEvent]:
    ...
    for event in events:
        previous = newest.get(event.idempotency_key)
...
def readiness_status(probes: Iterable[dict[str, Any]]) -> str:
    ...
    if any(
        not probe.get("ready", False) and probe.get("mandatory", False)
        for probe in probe_list
    ):
        return "not_ready"
    if any(not probe.get("ready", False) for probe in probe_list):
        return "degraded"
    return "ready"
